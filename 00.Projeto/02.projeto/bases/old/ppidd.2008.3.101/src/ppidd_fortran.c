/* src/mpp/ppidd_fortran.c $Revision: 2008.3 Patch(2008.3): mpp_globalop2 $ */
 ...
 * the definitions in the ppidd_fortran.h header file.                    *
 *                                                                        *
...
/*! \file
 * \brief This file contains the Fortran interface routines to PPIDD */
...
/* call fortran interface routine PBEGINF_() instead of c interface routine PBEGIN_(argc, argv) to avoid handling the arguments additionally */
   ...
   /* Also need to free argv itself */
      if (argv) free(argv);
  #ifdef MPI2
      if(MPIGA_Debug)printf("%4d: In mpiga_fortran: end of mpiga_initialize.\n",mpigv(myproc));
  #endif
#endif
   }
